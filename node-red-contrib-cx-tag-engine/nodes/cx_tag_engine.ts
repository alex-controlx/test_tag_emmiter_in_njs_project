...
export default class CxTagEngineNode {
    static RED: NodeAPI;
    private static tagStorages: {[key: string]: TagStorage} = {};

    static subscribeToTagChanges(tagPath: string, tagName: string, cb: (tag: Tag) => void) {
        const tagStorage = CxTagEngineNode.tagStorages['default'];

        eventEmitter.on(tagPath + '/' + tagName, (tagConf: ITagEmitConfig) => {
    ...
    static setTag(parentPath: string, tagName: string, newValue: any) {
        const tagStorage = CxTagEngineNode.tagStorages['default'];
        ...
        const tagConfig: ITagEmitConfig = { path: parentPath, name: tagName };
    ...
    static storageConfig(config: IStorageManagerConfig, node: IStorageManagerNode) {
        ...
        // @ts-ignore
        if (CxTagEngineNode.RED.util.cxGetTag) return;
        // @ts-ignore
        CxTagEngineNode.RED.util.cxGetTag = function (tagName?: any, path?: any, storageName?: any) {
            if (!tagName) return;
    ...
    static valueEmitter(config: IValueEmitterConfig, node: Node) {
        ...
        const configNode: IStorageManagerNode = CxTagEngineNode.RED.nodes.getNode(configNodeId);
        ...
        if (config.isToEmitAllChanges) {
            ...
            if (config.emitOnStart) CxTagEngineNode.RED.events.on('flows:started', emitOnStart);
