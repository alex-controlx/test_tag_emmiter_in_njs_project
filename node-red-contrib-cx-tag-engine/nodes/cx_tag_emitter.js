...
module.exports = function (RED) {
    ...
    RED.httpAdmin.get('/__cx_tag_emitter/get_paths', (req, res) => {
        ...
        if (!configNodeId || !configNode || !configNode.tagStorage) {
            ...
            return res.json([cx_tag_engine_1.ROOT_STORAGE_PATH]).end();
        }
        const storage = (storageName)
            ? cx_tag_engine_1.TagStorage.getStoragesByGlobalContext(configNode, storageName)
    ...
    function StorageConfig(config) {
        ...
        const node = this;
        cx_tag_engine_1.default.storageConfig(config, node);
    }
    function ValueEmitter(config) {
        RED.nodes.createNode(this, config);
        const node = this;
        cx_tag_engine_1.default.valueEmitter(config, node);
    }
    function TagsIn(config) {
        RED.nodes.createNode(this, config);
        const node = this;
        cx_tag_engine_1.default.tagsIn(config, node);
    }
