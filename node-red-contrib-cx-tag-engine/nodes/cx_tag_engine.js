...
exports.TagStorage = exports.ROOT_STORAGE_PATH = exports.TAGS_STORAGE = void 0;
...
class CxTagEngineNode {
    static subscribeToTagChanges(tagPath, tagName, cb) {
        const tagStorage = CxTagEngineNode.tagStorages['default'];
        eventEmitter.on(tagPath + '/' + tagName, (tagConf) => {
    ...
    static storageConfig(config, node) {
        ...
        if (CxTagEngineNode.RED.util.cxGetTag)
            return;
        CxTagEngineNode.RED.util.cxGetTag = function (tagName, path, storageName) {
            if (!tagName)
                return;
    ...
    static valueEmitter(config, node) {
        ...
        const configNode = CxTagEngineNode.RED.nodes.getNode(configNodeId);
        ...
        if (config.emitOnStart) {
            const listenerCounts = CxTagEngineNode.RED.events.getMaxListeners() + 1;
            CxTagEngineNode.RED.events.setMaxListeners(listenerCounts);
        }
        ...
        if (config.isToEmitAllChanges) {
            ...
            if (config.emitOnStart)
                CxTagEngineNode.RED.events.on('flows:started', emitOnStart);
        ...
        function emitOnStart() {
            emitCurrentTags();
            CxTagEngineNode.RED.events.removeListener('flows:started', emitOnStart);
            const listenerCounts = (CxTagEngineNode.RED.events.getMaxListeners() - 1) < 10
                ? 10
                : CxTagEngineNode.RED.events.getMaxListeners() - 1;
            CxTagEngineNode.RED.events.setMaxListeners(listenerCounts);
        }
...
exports.default = CxTagEngineNode;
CxTagEngineNode.tagStorages = {};
...
//# sourceMappingURL=cx_tag_engine.js.map